...
def main():
    ...
    def find_path(start, goal,built):
        sr, sc = start
        gr, gc = goal
        if start == goal:
            return [start]
    ...
    def generate_path_commands(path,built):
        cmds = []
        L = len(path)
        r, c = path[0]
    ...
    def get_detour_commands(home, work,built):
        path = find_path(home, work,built)
        if path is None:
            return []
    ...
    def simulated_annealing():
        nonlocal best_score, best_commands
        funds = K
        ...
        for turn in range(T):
            ...
            if not pending and len(pep_t) > 0:
                ...
                idx = 0
                if random.random() < .6 and len(pep_t) > 1:
                    idx = random.choice(range(0, min(len(pep_t), 8)))

                if idx < len(pep_t):
                    home, work = pep_t[idx]
                    del pep_t[idx]
                    cmds = get_detour_commands(home, work,built)
                else:
                    cmds = []
                if cmds:
                    pending = cmds
